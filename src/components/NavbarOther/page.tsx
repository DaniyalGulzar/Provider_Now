...
const NavbarOther: React.FC = () => {
	...
	useEffect(() => {
		if (status === "authenticated" && session) {
			if (session.user) setFirstName(session.user?.first_name || null);
		}
	...
	return (
		<>
			<nav className="sticky top-0 z-10 bg-gradient-to-b from-[#491161] to-[#721A97] py-3 flex flex-col justify-between items-center rounded-b-[40px]">
				...
				<div
					className={`w-full ${isProviderOpen ? "block" : "hidden"} 1250:hidden`}
					id="provider-navbar"
				>
					<ul className="mb-0 sm:ml-0 lg:ml-5 md:ml-5 font-medium flex flex-col py-4 lg:flex-row lg:space-x-3 lg:mt-0 lg:border-0 text-white list-none">
						...
						<span className="hidden lg:block mx-4 h-6 w-px bg-gray-300 dark:bg-gray-600"></span>
						<li className="m-0-imp whitespace-nowrap flex">
							{status === "authenticated" ? (
								<div className="relative inline-block">
								<span
